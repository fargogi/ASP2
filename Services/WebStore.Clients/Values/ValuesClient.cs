...
namespace WebStore.Clients.Values
{
    public class ValuesClient : BaseClient, IValueService
    {
        ...
        {
            var result = await _httpClient.GetAsync(ServiceAddress);
        ...
        {
            var result = await _httpClient.GetAsync($"{ServiceAddress}/get/{id}");
            if (result.IsSuccessStatusCode)
        ...
        {
            var result = await _httpClient.PostAsJsonAsync($"{ServiceAddress}/post", value);
        ...
        {
            var result = await _httpClient.PutAsJsonAsync($"{ServiceAddress}/put/{id}", value);
            result.EnsureSuccessStatusCode();
        ...
        {
            var result = await _httpClient.DeleteAsync($"{ServiceAddress}/delete/{id}");
            return result.StatusCode;
        }
    }
}
